...
<script>
    ...
    //查看企业业主详细信息
    function showInfo(ownerId){
        if($('#enterPrisetb').tabs('tabs').length>0){
            for(var i=0;i<$('#enterPrisetb').tabs('tabs').length;i++) {
                var title= $('#enterPrisetb').tabs('getTab',i).panel('options').title;
                var ti=title.split("-")[0]
                if(ti=="企业业主详细信息"){
                    $("#enterPrisetb").tabs("close",title);
                }
            }
            $("#enterPrisetb").tabs('add',{
                title:"企业业主详细信息-"+ownerId,
                href:basePath+"/enterpriseOWner/toOneEnterpriseOwnerView.do?ownerId="+ownerId,
                closable:true
            });
        }
    }
